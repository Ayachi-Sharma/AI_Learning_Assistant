        ...
        <div className='relative border-b-2 border-slate-100'>
            <nav className='flex gap-2'>
                ...
                className={`relative pb-4 px-2 md:px-6 text-sm font-semibold transition-all duration-200 ${
                    activeTab === tab.name
                    ? 'text-violet-700 tracking-wider'
                    : 'text-slate-600 hover-text-slate-900'
                }`}>
                    <span className='relative z-10'>{tab.label}</span>
                    {activeTab === tab.name && (
                        <div className='absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-indigo-500 to-violet-500 rounded-full shadow-lg shadow-indigo-500/25'/>
                    )}
                    {activeTab === tab.name && (
