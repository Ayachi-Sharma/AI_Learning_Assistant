...
const QuizCard = ({ quiz, onDelete }) => {
    return (
        <div className='group relative bg-white/80 backdrop-blur-xl border-2 border-slate-200 hover:border-indigo-300 rounded-2xl p-4 transition-all duration-200 hover:sahdow-lg hover:shadow-emerald-500/10 flex flex-col justify-between'>
            <button
                onClick={(e) => {
                    e.stopPropagation();
                    onDelete(quiz);
                ...
                <div className='inline-flex items-center gap-1.5 py-1 rounded-lg text-sm font-semibold'>
                    {quiz?.completedAt ? (
                        ...
                        <div className='flex items-center gap-1.5 bg-amber-50 border border-amber-200 rounded-lg px-3 py-1'>
                            <Play className='w-3.5 h-3.5 text-amber-600' strokeWidth={2.5} />
            ...
            <div className='mt-2 pt-4 border-t border-slate-100'>
                ...
                {quiz?.completedAt ? (
                    ...
                    <Link to={`/quizzes/${quiz._id}`}>
                        <button className='curson-pointer group/btn relative w-full h-11 bg-linear-to-r from-indigo-500 to-violet-400 hover:from-indigo-600 hover:to-violet-600 text-white font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-indigo-500/25 active:scale-95 overflow-hidden cursor-pointer'>
                            <span className='relative z-10 flex items-center justify-center gap-2'>
                                <Play className='w-4 h-4' strokeWidth={2.5} />
                                Start Quiz
                            </span>
                            <div className='absolute inset-0 bg-linear-to-r from-white/0 via-white/20 to-white/20 -translate-x-full group-hover/btn:translate-x-full transition-transform duration-700' />
                        </button>
                    </Link>
                )}
            </div>
